...
import { CollectionReference, DocumentReference, Firestore, addDoc, collection, collectionData, doc, getDocs, orderBy, query, updateDoc } from '@angular/fire/firestore';
...
export class CosasFeasPage implements OnInit {
  ...
  constructor(
    ...
    this.publicacionesCollection = collection(this.firestore, 'posts/cosas_feas/publicaciones')
  ...
  ngOnInit() {
    this.verVentanaConfirmacion.subscribe(() => {this.chRef.detectChanges()})
    return
  }
  ...
  async subirArchivoStorage(photo: Photo) {
    ...
    const nombreArchivo = 'imagenes/cosas_feas/' + Date.now() + '.jpeg'
